...
function getanswer(query){    
    ...
    $.get("https://www.omdbapi.com/?t="+query+"&apikey=9758597e", function(rawdata){
        ...
        if (data.Error === "Movie not found!"){
            document.getElementById('answer').innerHTML = "<h2 class='msg'><br><br>Movie not found..</h2>"
            document.getElementById('more_results').innerHTML= "";
        }else if (data.Title===undefined){
            document.getElementById('answer').innerHTML = "<h2 class='msg'><br><br>Search for a movie..</h2>"
            document.getElementById('more_results').innerHTML= "";
        }else{
...
function  loadMovie(ident) {
    document.getElementById('result').innerHTML="";
    document.getElementById(ident).innerHTML = "";

    //var movies = document.getElementsByClassName('movie')
    const bookmarks = ident.split('@');

    var i =0;

    while(i<(bookmarks.length-1)) {
        $.get("https://www.omdbapi.com/?i=" + bookmarks[i] + "&apikey=9758597e", function (rawdata) {
            ...
            document.getElementById('result').innerHTML += "<div class='movie'><h1>" + title + "</h1> <br><img class='poster' src= '" + posterURL + "' alt='Poster'> <button class='bookmark_btn'      id='" + id + "' onclick = 'changeImg(this.id)'> <img id='" + id + "unchecked' class='bookmark' src='images/bookmarked.png' alt='Bookmark'       height='50px' width='50px'> </button> <br><p>Year Released: " + year + "</p> <p>Duration: " + duration + "</p> <p>Genre: " + genre + "</p> <p>Director: " + director + "</p> <p>Actors: " + actors + "</p> <p>IMDB page:      <a href='" + imdbURL + "'target='_blank'>" + imdbURL + "</a></p>  <p     id='plot'>Plot: <br>" + plot + "</p></div>";
            document.getElementById('footer').style.position = "relative";
